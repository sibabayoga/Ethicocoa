...
def purchasing_page():
    ...
    try:
        ...
        vendors = []
    return render_template_string(PURCHASING_HTML, vendors=vendors, email=session['email'], active='purchasing')
...
def get_products_by_vendor(vendor_id):
    ...
    try:
        try:
            ...
            supplier_infos = models.execute_kw(ODOO_DB, session['uid'], session['password'], 'product.supplierinfo', 'search_read', [[['name', '=', vendor_id]]], {'fields': ['product_id', 'product_tmpl_id', 'price']})
        ...
        for s in supplier_infos:
            ...
            if s.get('product_id'):
                ...
                variants = models.execute_kw(ODOO_DB, session['uid'], session['password'], 'product.product', 'search_read', [[['product_tmpl_id', '=', tmpl_id]]], {'fields': ['id', 'name', 'standard_price', 'list_price']})
                for p in variants:
                    if p['id'] not in seen:
                        seen.add(p['id'])
                        fp = harga_vendor or p.get('standard_price', 0) or p.get('list_price', 0) or 25000
                        product_list.append({'id': p['id'], 'name': p['name'], 'price': fp})
...
def create_po_advanced():
    ...
    _, models = get_odoo_client()
    try:
        # 1. Membuat draf RFQ baru di Odoo
        po_id = models.execute_kw(
            ODOO_DB, session['uid'], session['password'], 
            'purchase.order', 'create', 
            [{
                'partner_id': int(request.form['vendor_id']), 
                'date_order': request.form['deadline'] + ' 00:00:00'
            }]
        )
        
        # 2. Memasukkan baris produk ke dalam draf RFQ tersebut
        models.execute_kw(
            ODOO_DB, session['uid'], session['password'], 
            ...
            [{
                ...
                'product_qty': float(request.form['qty']), 
                'price_unit': float(request.form['price']), 
                'name': 'Pembelian Material Baku', 
                'date_planned': request.form['deadline'] + ' 00:00:00'
            }]
        )
        
        # BARIS BUTTON_CONFIRM DI SINI SUDAH DIHAPUS SUPAYA STATUSNYA TETAP RFQ DAN BISA DI-ACC MANUAL DI ODOO
        
        flash(f'Dokumen RFQ #{po_id} berhasil dibuat. Silakan lakukan validasi / ACC langsung di dalam Odoo.', 'success')
    except Exception as e:
        flash(f'Gagal menerbitkan dokumen RFQ ke Odoo: {str(e)}', 'danger')
